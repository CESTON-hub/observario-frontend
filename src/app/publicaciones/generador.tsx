...
import { indicadores, regiones, coberturaRegiones } from "@/lib/data";
...
const territoriosDisponibles = ["La Guajira", "Bogotá", "Medellín", "Barranquilla", "Cali"];
...
function construirHTML(territorio: string) {
  ...
  const region = Object.keys(coberturaRegiones).find((r) => ["la guajira", "barranquilla"].some((x) => t.includes(x)) ? r === "Caribe" : t.includes("medellin") ? r === "Andina" : t.includes("cali") ? r === "Pacífico" : r === "Andina");
...
<h1>Observatorio de Datos ACIEM — Perfil territorial: ${territorio}</h1>
...
<table>${filas([
    ["Entidades del directorio con presencia o proyectos", String(orgs.length)],
    ["Proyectos activos o planeados en el territorio", String(proyectos.length)],
    ["Convocatorias vigentes aplicables", String(convs.length)],
    [`Cobertura eléctrica de la región (${region ?? "—"})`, region ? `${coberturaRegiones[region]}%` : "—"],
  ])}</table>
<h2>Entidades</h2>
<table><tr><th style="text-align:left;padding:6px 10px">Entidad</th><th style="text-align:left;padding:6px 10px">Sector</th></tr>
${filas(orgs.map((o) => [o.razonSocial, o.sector]))}</table>
...
<h2>Indicadores nacionales de referencia</h2>
<table>${filas(indicadores.slice(0, 4).map((i) => [i.nombre, `${i.valor} ${i.unidad} (${i.variacion > 0 ? "+" : ""}${i.variacion}% interanual)`]))}</table>
<p class="meta">Fuente: Observatorio de Datos ACIEM. Este perfil se genera automáticamente desde la base de evidencias; las cifras se verifican contra los datos fuente antes de su publicación.</p>
</body></html>`;
}
